...
export default function CalculatorsRoute() {
  ...
  const processChecksum = function (data) {
    ...
    let checkSum;
    if (_inMenuItem === 0) checkSum = helpers.fletcherDM(dataArray);
    if (_inMenuItem === 1) checkSum = helpers.crc16_ccitt(dataArray);
    if (_inMenuItem === 2) checkSum = helpers.crc16_ibm(dataArray);
    if (_inMenuItem === 3) checkSum = helpers.crc16_x25(dataArray);

    const checkSumArr = helpers.numToFixedSizeArr(checkSum);
